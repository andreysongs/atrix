    ...
    <motion.div className="forge-onboarding" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <section className="onboarding-visual">
        ...
        <div className="onboarding-visual-copy">
          <span className="forge-mini-brand"><i>F</i> FORGE</span>
          <h1>Construa sua<br />melhor versão.</h1>
          <p>Treino, recuperação e progresso conectados em uma experiência feita para você.</p>
      ...
      <section className="onboarding-panel">
        ...
        {step === 0 && (
          <motion.div className="onboarding-step" initial={{ x: 16, opacity: 0 }} animate={{ x: 0, opacity: 1 }}>
            ...
            <p className="eyebrow">BEM-VINDO AO FORGE</p>
            <h2>Seu plano começa com o que importa.</h2>
            <p>Conte seu objetivo e o FORGE organiza treino, progressão e recuperação em uma jornada simples.</p>
        ...
        {step === 2 && (
          <motion.div className="onboarding-step" initial={{ x: 16, opacity: 0 }} animate={{ x: 0, opacity: 1 }}>
            ...
            <p>Você poderá mudar tudo depois. O FORGE não substitui avaliação profissional.</p>
            ...
            <div className="onboarding-actions"><button onClick={() => setStep(1)}>Voltar</button><button className="forge-main-action" onClick={() => onComplete({ name, goal, place, limitation })}>Entrar no FORGE <ArrowRight size={18} /></button></div>
          </motion.div>
        )}
      </section>
    </motion.div>
...
export function RunningView({ toast }: { toast: (message: string) => void }) {
  ...
  return (
    <div className="forge-module-view">
      <header className="module-heading"><div><p className="eyebrow">FORGE RUN</p><h1>Sua corrida, mais inteligente.</h1><p>Ritmo, volume e recuperação em uma visão que ajuda você a avançar.</p></div><button className="forge-main-action" onClick={() => toast("GPS preparado. Inicie quando estiver pronto.")}><Footprints size={18} /> Iniciar corrida</button></header>
...
export function TrainerView({ toast }: { toast: (message: string) => void }) {
  ...
  return (
    <div className="forge-module-view trainer-view">
      <header className="module-heading"><div><p className="eyebrow">FORGE PRO</p><h1>Bom trabalho, Andrey.</h1><p>Seus alunos estão com 88% de aderência média nesta semana.</p></div><button className="forge-main-action" onClick={() => toast("Cadastro de aluno preparado.")}><Plus size={18} /> Novo aluno</button></header>
