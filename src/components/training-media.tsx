...
export function ProgramsView({ onStart, toast }: { onStart: (workout: Workout) => void; toast: (message: string) => void }) {
  ...
  useEffect(() => {
    try { setDownloads(JSON.parse(localStorage.getItem("pulse-guided-downloads") || "[]") as string[]); } catch { setDownloads([]); }
  }, []);
  ...
  const toggleDownload = async (session: GuidedSession) => {
    ...
    setDownloads(next);
    localStorage.setItem("pulse-guided-downloads", JSON.stringify(next));
    if (!exists && "caches" in window) {
      const cache = await caches.open("pulse-guided-media-v1");
  ...
  return (
    <div className="programs-view">
      <section className="programs-masthead">
        ...
        <div><span className="editorial-kicker"><i /> PULSE TRAINING</span><h1>Movimento que<br />muda você.</h1><p>Programas progressivos, sessões guiadas e técnica para cada fase da sua evolução.</p><button onClick={() => setPlayingSession(guidedSessions[0])}><Play size={17} fill="currentColor" /> Assistir treino</button></div>
      </section>
